...
export type Role = 'SUPER_ADMIN' | 'ADMIN' | 'MANAGER' | 'RECEPTIONIST' | 'HOUSEKEEPING' | 'CASHIER' | 'BARTENDER' | 'STAFF';

export type RoomStatus = 'AVAILABLE' | 'OCCUPIED' | 'RESERVED' | 'HOUSEKEEPING' | 'MAINTENANCE' | 'OUT_OF_ORDER';
...
export interface User {
  id: string; email: string; username: string; role: Role;
  isActive: boolean; staffId?: string; createdAt: string; lastLogin?: string;
}

export interface Staff {
  id: string; userId?: string; employeeCode: string;
  firstName: string; lastName: string; phone?: string; email?: string;
  department: string; position: string; isActive: boolean; joinDate: string;
}

export interface Room {
  id: string; number: string; floor: number; type: RoomType;
  status: RoomStatus; baseRate: number; capacity: number;
  beds: string; amenities: string[]; notes?: string;
}

export interface Guest {
  id: string; firstName: string; lastName: string; email?: string;
  phone: string; nationality?: string; idType?: string;
  idNumber?: string; isVip: boolean; notes?: string;
}

export interface Reservation {
  id: string; guestId: string; roomId: string;
  checkIn: string; checkOut: string; nights: number;
  adults: number; children: number; status: ReservationStatus;
  source: BookingSource; ratePerNight: number;
  totalAmount: number; amountPaid: number;
  notes?: string; createdAt: string;
}

export interface FolioItem {
  id: string; reservationId: string; date: string;
  description: string; category: 'ROOM' | 'FB' | 'SERVICE' | 'PAYMENT' | 'OTHER';
  amount: number; createdBy: string;
}

export interface AttendanceRecord {
  id: string; staffId: string; type: AttendanceType;
  timestamp: string; method: 'MANUAL' | 'BIOMETRIC' | 'CARD'; note?: string;
}
