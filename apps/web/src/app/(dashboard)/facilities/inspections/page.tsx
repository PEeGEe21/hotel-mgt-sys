...
const inspectionTypes: Array<'All' | InspectionType> = ['All', 'INTERNAL', 'THIRD_PARTY', 'REGULATORY'];
...
export default function FacilityInspectionsPage() {
  ...
  const { data, isLoading, error: loadError } = useFacilityInspections({
  ...
  return (
    <div className="space-y-6">
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            ...
            {(error || loadError) && (
              <span className="ml-2 text-xs text-red-400">
                {error ?? (loadError instanceof Error ? loadError.message : 'Failed to load inspections')}
              </span>
            )}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Total', count: data?.total ?? inspections.length, color: 'text-blue-400', bg: 'bg-blue-500/10 border-blue-500/20' },
          { label: 'Submitted', count: inspections.filter((i) => i.status === 'SUBMITTED').length, color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20' },
          { label: 'In Progress', count: inspections.filter((i) => i.status === 'IN_PROGRESS').length, color: 'text-amber-400', bg: 'bg-amber-500/10 border-amber-500/20' },
          { label: 'Scheduled', count: inspections.filter((i) => i.status === 'SCHEDULED').length, color: 'text-blue-400', bg: 'bg-blue-500/10 border-blue-500/20' },
      ...
      <div className="bg-[#161b27] border border-[#1e2536] rounded-xl overflow-x-auto">
        <table className="w-full min-w-[900px]">
          <thead className="border-b border-[#1e2536] bg-[#0f1117]/50">
            <tr>
              {['#', 'Facility', 'Type', 'Inspector', 'Scheduled By', 'Date', 'Score', 'Status', 'Notes'].map((h) => (
          ...
          <tbody>
            ...
            {inspections.map((insp, i) => (
              <tr key={insp.id} className="border-b border-[#1e2536] last:border-0 hover:bg-white/[0.02] transition-colors">
                ...
                <td className="px-4 py-3 text-xs text-slate-400 whitespace-nowrap">{insp.inspectionType}</td>
                ...
                <td className="px-4 py-3 text-xs text-slate-400 whitespace-nowrap">{toDate(insp.scheduledAt)}</td>
                <td className="px-4 py-3">
                  {Number(insp.score ?? 0) > 0 ? (
                    <span className={`text-sm font-bold ${Number(insp.score) >= 80 ? 'text-emerald-400' : Number(insp.score) >= 60 ? 'text-amber-400' : 'text-red-400'}`}>
                      {insp.score}%
                    </span>
                  ) : <span className="text-slate-600 text-sm">—</span>}
                </td>
                <td className="px-4 py-3">
                  <span className={`text-xs px-2.5 py-1 rounded-full font-medium flex items-center gap-1.5 w-fit ${statusStyle[insp.status] ?? 'bg-slate-500/15 text-slate-400'}`}>
                    <StatusIcon status={insp.status} />
                    {insp.status}
                  </span>
                </td>
                <td className="px-4 py-3 text-xs text-slate-500 max-w-[220px] truncate">{insp.findings || '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#161b27] border border-[#1e2536] rounded-2xl p-6 w-full max-w-lg shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-xs text-slate-500 uppercase tracking-wider mb-1.5 block">Inspection Type</label>
                <select value={form.inspectionType} onChange={(e) => setForm({ ...form, inspectionType: e.target.value })} className="w-full bg-[#0f1117] border border-[#1e2536] rounded-lg px-3 py-2.5 text-sm text-slate-200">
              ...
              <div>
                <label className="text-xs text-slate-500 uppercase tracking-wider mb-1.5 block">Scheduled By</label>
                <select value={form.scheduledBy} onChange={(e) => setForm({ ...form, scheduledBy: e.target.value })} className="w-full bg-[#0f1117] border border-[#1e2536] rounded-lg px-3 py-2.5 text-sm text-slate-200">
              ...
              <div>
                <label className="text-xs text-slate-500 uppercase tracking-wider mb-1.5 block">Inspector</label>
                <input value={form.inspectorName} onChange={(e) => setForm({ ...form, inspectorName: e.target.value })} placeholder="Inspector name" className="w-full bg-[#0f1117] border border-[#1e2536] rounded-lg px-3 py-2.5 text-sm text-slate-200 placeholder:text-slate-600" />
              </div>
              <div>
                <label className="text-xs text-slate-500 uppercase tracking-wider mb-1.5 block">Organization</label>
                <input value={form.inspectorOrganization} onChange={(e) => setForm({ ...form, inspectorOrganization: e.target.value })} placeholder="Optional" className="w-full bg-[#0f1117] border border-[#1e2536] rounded-lg px-3 py-2.5 text-sm text-slate-200 placeholder:text-slate-600" />
              </div>
              <div>
                <label className="text-xs text-slate-500 uppercase tracking-wider mb-1.5 block">Date</label>
                <input type="date" value={form.scheduledAt} onChange={(e) => setForm({ ...form, scheduledAt: e.target.value })} className="w-full bg-[#0f1117] border border-[#1e2536] rounded-lg px-3 py-2.5 text-sm text-slate-200" />
              </div>
              <div className="col-span-2">
                <label className="text-xs text-slate-500 uppercase tracking-wider mb-1.5 block">Scope / Notes</label>
                <input value={form.findings} onChange={(e) => setForm({ ...form, findings: e.target.value })} placeholder="Inspection scope or notes" className="w-full bg-[#0f1117] border border-[#1e2536] rounded-lg px-3 py-2.5 text-sm text-slate-200 placeholder:text-slate-600" />
              </div>
            </div>
