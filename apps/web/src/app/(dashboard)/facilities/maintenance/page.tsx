...
type MaintenanceStatus = 'OPEN' | 'ASSIGNED' | 'IN_PROGRESS' | 'PENDING_PARTS' | 'RESOLVED' | 'CLOSED';
...
const categories = ['PLUMBING', 'ELECTRICAL', 'HVAC', 'FURNITURE', 'EQUIPMENT', 'STRUCTURAL', 'OTHER'];
...
export default function FacilityMaintenancePage() {
  ...
  const { data, isLoading, error: loadError } = useFacilityMaintenance({
  ...
  return (
    <div className="space-y-6">
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            ...
            {(error || loadError) && (
              <span className="ml-2 text-xs text-red-400">
                {error ?? (loadError instanceof Error ? loadError.message : 'Failed to load maintenance')}
              </span>
            )}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Open', count: records.filter((r) => r.status === 'OPEN').length, color: 'text-blue-400', bg: 'bg-blue-500/10 border-blue-500/20' },
          { label: 'In Progress', count: records.filter((r) => r.status === 'IN_PROGRESS').length, color: 'text-amber-400', bg: 'bg-amber-500/10 border-amber-500/20' },
          { label: 'Resolved', count: records.filter((r) => r.status === 'RESOLVED').length, color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20' },
      ...
      <div className="bg-[#161b27] border border-[#1e2536] rounded-xl overflow-x-auto">
        <table className="w-full min-w-[900px]">
          <thead className="border-b border-[#1e2536] bg-[#0f1117]/50">
            <tr>
              {['#', 'Task', 'Category', 'Facility', 'Assigned To', 'Requested', 'Completed', 'Priority', 'Status', 'Cost'].map((h) => (
            ...
            {records.length === 0 && (
              <tr className="border-b border-[#1e2536] last:border-0 text-center">
                <td colSpan={10} className="px-4 py-3 text-sm text-slate-500">
                  {isLoading ? 'Loading maintenance requests...' : 'No maintenance requests found'}
                </td>
              </tr>
            )}
            {records.map((r, i) => (
              <tr key={r.id} className="border-b border-[#1e2536] last:border-0 hover:bg-white/[0.02] transition-colors">
                ...
                <td className="px-4 py-3 text-xs text-slate-400 whitespace-nowrap">{r.category}</td>
                <td className="px-4 py-3 text-sm text-slate-400 whitespace-nowrap">{r.facility?.name ?? '—'}</td>
                <td className="px-4 py-3 text-sm text-slate-400 whitespace-nowrap">{staffName(r.assignedToStaff)}</td>
                <td className="px-4 py-3 text-xs text-slate-500 whitespace-nowrap">{toDate(r.createdAt)}</td>
                <td className="px-4 py-3 text-xs text-slate-400 whitespace-nowrap">{toDate(r.resolvedAt ?? r.closedAt)}</td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${statusStyle[r.status] ?? 'bg-slate-500/15 text-slate-400'}`}>{r.status}</span>
                </td>
                <td className="px-4 py-3 text-sm text-slate-300 whitespace-nowrap">₦{maintenanceCost(r).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <div className="bg-[#161b27] border border-[#1e2536] rounded-2xl p-6 w-full max-w-lg shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <label className="text-xs text-slate-500 uppercase tracking-wider mb-1.5 block">Task Title</label>
                <input
                  value={form.title}
              ...
              <div>
                <label className="text-xs text-slate-500 uppercase tracking-wider mb-1.5 block">Assigned To</label>
                <select
              ...
              <div>
                <label className="text-xs text-slate-500 uppercase tracking-wider mb-1.5 block">Estimated Mins</label>
                <input
                  type="number"
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button
                ...
                disabled={createMaintenance.isPending || !form.title.trim() || !form.description.trim()}
