...
export default function DeleteConfirm({
  ...
  const handleDelete = async () => {
    try {
      ...
      openToast('success', `${terminal?.name} deleted`);
      onClose();
    } catch (e: any) {
      const msg = e?.response?.data?.message ?? 'Could not delete terminal.';
      setError(msg);
      openToast('error', msg);
    }
  };
    ...
    >
      ...
      <DialogContent className="bg-[#161b27] border border-[#1e2536] ring-0 !outline-none rounded-2xl w-full max-w-sm shadow-2xl p-6 space-y-4">
        ...
        <div className="text-center">
          <h3 className="text-base font-bold text-white">Delete {terminal?.name}?</h3>
        ...
        <div className="flex gap-3">
          ...
          >
            {deleteTerminal.isPending && <Loader2 size={14} className="animate-spin" />}
            Delete
          </button>
        </div>
      </DialogContent>
