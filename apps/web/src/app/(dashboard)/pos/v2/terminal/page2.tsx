...
export default function PosTerminalPage() {
  ...
  // Persisted cart — survives page refresh for up to 8 hours
  const {
    items,
    carts,
    activeTable,
  ...
  // ── Data ───────────────────────────────────────────────────────────────────
  // Set initial table when tables load (only if cart hook hasn't restored one)
  useEffect(() => {
  ...
  // ── Cart helpers ───────────────────────────────────────────────────────────
  const addItem = (product: any) => cartAddItem(product);
  ...
  const handlePay = async (method: PayMethod, amountTendered?: number) => {
    ...
    try {
      ...
      const isWalkup = activeTable === 'Walkup' || activeTableObj?.section === 'Takeaway';
  ...
  const tablesWithItems = Object.entries(carts)
    .filter(([, items]) => items.length > 0)
    .map(([table]) => table);

  // Don't render until cart is hydrated from sessionStorage
  if (!hydrated)
    return (
      <div className="flex items-center justify-center h-[calc(100vh-4rem)] bg-[#0f1117]">
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] bg-[#0f1117] overflow-hidden -m-6">
      ...
      {view === 'order' && (
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-72 xl:w-80 flex flex-col border-l border-[#1e2536] bg-[#0f1117] shrink-0">
            ...
            <div className="px-4 pt-3 pb-2 border-b border-[#1e2536] shrink-0 max-h-48 overflow-y-auto">
                      ...
                      {section.tables.map((t: PosTable) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {t.capacity ? (
                              <span className="text-[9px] ml-1 opacity-50">{t.capacity}</span>
                            ) : null}
                          </button>
                        );
                      })}
              ...
              {items.length > 0 ? (
                <>
                  ...
                  <button
                    ...
                    disabled={
                      createOrder.isPending || deliverOrder.isPending || payOrderById.isPending
                    }
      ...
      {showPay && (
        <PaymentModal
          ...
          onConfirm={handlePay}
          isLoading={createOrder.isPending || deliverOrder.isPending || payOrderById.isPending}
        />
      )}
      {showRoom && (
        <RoomChargeModal
