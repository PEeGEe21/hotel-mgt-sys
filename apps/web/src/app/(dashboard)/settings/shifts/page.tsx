...
type Shift = { id: string; name: string; startTime: string; endTime: string; color: string; days: string[] };
...
const initShifts: Shift[] = [
  { id: 'sh1', name: 'Morning', startTime: '07:00', endTime: '15:00', color: 'bg-amber-500', days: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'] },
  { id: 'sh2', name: 'Evening', startTime: '15:00', endTime: '23:00', color: 'bg-violet-500', days: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'] },
  { id: 'sh3', name: 'Night', startTime: '23:00', endTime: '07:00', color: 'bg-slate-500', days: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'] },
  { id: 'sh4', name: 'Half Day', startTime: '08:00', endTime: '13:00', color: 'bg-emerald-500', days: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'] },
];
...
export default function ShiftsPage() {
  ...
  const save = (data: Omit<Shift, 'id'>) => {
    if (modal.shift) {
      setShifts(s => s.map(x => x.id === modal.shift!.id ? { ...x, ...data } : x));
    } else {
      setShifts(s => [...s, { ...data, id: Date.now().toString() }]);
    }
    setModal({ open: false });
  };
            ...
            <div key={shift.id} className="flex items-center justify-between px-5 py-4 hover:bg-white/[0.02] transition-colors">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex gap-1">
                  ...
                  <button onClick={() => setShifts(s => s.filter(x => x.id !== shift.id))}
